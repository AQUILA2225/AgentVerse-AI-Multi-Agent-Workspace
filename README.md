...
** Working Flow
User selects an AI agent from the sidebar.
The selected agent opens a dedicated chat session.
User interacts with the AI through a ChatGPT-style interface.
Previous memory is loaded from backend storage.
Relevant information is retrieved from the vector database using RAG.
The LLM generates a personalized response using:
Current user query
Previous conversation memory
Retrieved knowledge
Agent-specific instructions
The response is displayed in the UI and saved to memory.
Learning Outcomes
